const Timeline = () => {
  ...
  return (
    <section id="timeline" className="p-8 max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold mb-6">Timeline</h2>
      <ol className="border-l-4 border-blue-500">
        {events.map((e, i) => (
          <li key={i} className="p-4 cursor-default hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors">
            <div className="font-semibold text-lg">
              {e.year} — {e.title}
            </div>
            <p className="text-pretty text-gray-600 dark:text-gray-300">{e.desc}</p>
          </li>
        ))}
      </ol>
    </section>
